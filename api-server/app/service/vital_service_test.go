...
func Test_GetVitalsByPatientIDAndDateRange(t *testing.T) {
	beforeEachVital(t)
	...
	}{
		{
			...
			setupMock: func() {
				vitals := []vital.Vital{
					...
					{
						PatientID:  "P00001234",
						RecordedAt: time.Date(2025, 12, 1, 11, 15, 0, 0, time.UTC),
						VitalType:  "HR",
						Value:      115.0,
						Version:    1,
					},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := vitalSvc.GetVitalsByPatientIDAndDateRange(ctx, tt.req)
			...
			if tt.wantErr {
				...
				require.Equal(t, tt.wantCount, len(result.Items))

				// vital_type이 있을 때는 모든 item이 해당 타입이어야 함
				if tt.req.VitalType != "" {
					for _, item := range result.Items {
						require.Equal(t, tt.req.VitalType, item.VitalType)
					}
				}
			}
		})
